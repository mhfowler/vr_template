...
THREE.VRControls = function ( camera, done ) {
	...
	this._init = function () {
		...
		function key(event, sign) {
			var control = vr_controls.manualControls[event.keyCode];
			...
			if (self.isWASD && control.index <= 2){
				vr_controls.manualRotateRate[control.index] += sign * control.sign;
			} else if (self.isArrows && control.index <= 5) {
				vr_controls.manualMoveRate[control.index - 3] += sign * control.sign;
			}
		}
